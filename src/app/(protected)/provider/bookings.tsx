// src/app/(protected)/provider/bookings.tsx
...
import { ProviderBookingListItem } from '@/components/booking/ProviderBookingListItem';
...
export default function ProviderBookingsScreen() {
  ...
  const { data, isLoading, isError, refetch } = useMyBookings('provider');

  const bookings = data?.items ?? [];

  const renderItem = useCallback(
    ({ item }: { item: Booking }) => <ProviderBookingListItem booking={item} />,
    [],
  );
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: t.background }]} edges={['top']}>
      <View style={styles.header}>
        ...
        <ThemedText style={[styles.headerTitle, { color: t.text }]}>
          {translate('provider.requestsTitle')}
        </ThemedText>
      </View>
      ...
      {isLoading ? (
        ...
        <View style={styles.centered}>
          ...
          <ThemedText style={[styles.emptyTitle, { color: t.text }]}>
            {translate('provider.requestsLoadError')}
          </ThemedText>
          <TouchableOpacity
        ...
        <View style={styles.centered}>
          <MaterialCommunityIcons name="clipboard-text-outline" size={40} color={t.icon} />
          <ThemedText style={[styles.emptyTitle, { color: t.text }]}>
            {translate('provider.requestsEmptyTitle')}
          </ThemedText>
          <ThemedText style={[styles.emptySubtext, { color: t.icon }]}>
            {translate('provider.requestsEmptySubtitle')}
          </ThemedText>
        </View>
      ) : (
        <FlatList
